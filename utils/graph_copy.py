...
def extract_1hop_subgraphs_per_qc(data):
    """
    Given a set of question concept IDs, return a list of 1-hop subgraphs:
        [(adj, concept_ids, qmask), ...]
    Each subgraph is centered on one question concept.
    """
    qc_ids, _ = data
    ...
    for cid in qc_ids:
        ...
        if cid in cpnet.nodes:
            # gather 1-hop neighbors
            extra_nodes |= set(cpnet[cid]) #list(cpnet[cid].keys())
            schema_graph = [cid] + sorted(extra_nodes)

            # build adjacency and mask
            adj, concept_ids = concepts2adj(schema_graph)
            qmask = np.arange(len(schema_graph)) == 0

#         extra_nodes = list(cpnet[cid].keys())
#         schema_graph = sorted({cid} | set(extra_nodes))

#         # build adjacency and mask
#         adj, concept_ids = concepts2adj(schema_graph)
#         idx = schema_graph.index(cid)
#         qmask = np.arange(len(schema_graph)) == idx

            subgraphs.append((adj, concept_ids, qmask))
...
def generate_adj_data_from_grounded_concepts_paper1(grounded_path, cpnet_graph_path, cpnet_vocab_path, output_path, num_processes):
    """
...
#####################################################################################################
def generate_adj_data_from_grounded_concepts_paper2(grounded_path, cpnet_graph_path, cpnet_vocab_path, output_path, num_processes):
    """
    This function will save
    ...
    if cpnet is None:
        print("CPNET is still none")
    qa_data = []
    with open(grounded_path, 'r', encoding='utf-8') as fin:
        for line_id, line in enumerate(fin):
            if line_id % 5 != 0:
                continue  # Only take the first grounded record per question
            dic = json.loads(line)
            q_ids = set(concept2id[c] for c in dic['qc'])
            a_ids = set(concept2id[c] for c in dic['ac'])
            q_ids = q_ids - a_ids
    ...
    # Parallel extraction: one call per question (set of qc IDs)
    with Pool(num_processes) as p:
        res = list(tqdm(p.imap(extract_1hop_subgraphs_per_qc, qa_data), total=len(qa_data)))

    # Save the per-question, per-concept subgraphs
    with open(output_path, 'wb') as fout:
        pickle.dump(res, fout)
