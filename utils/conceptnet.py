...
def construct_graph(cpnet_csv_path, cpnet_vocab_path, output_path, prune=True):
    ...
    with open(cpnet_csv_path, "r", encoding="utf8") as fin:
        ...
        def not_save(cpt):
            ...
            words = cpt.split("_")  # ConceptNet uses "_" to separate words
            if len(words) == 1:  # Single-word concept
                if words[0] in merged_stopwords:
                    return True
            elif all(word in merged_stopwords for word in words):  # Multi-word concept (only remove if ALL parts are stopwords)
                return True

            if len(cpt) <= 2:  # Remove very short, non-meaningful words
                return True
            return False
