...
# Load SpaCy model for stopword handling
...
# Merge stopwords from multiple libraries
...
merged_stopwords = nltk_stopwords | spacy_stopwords | sklearn_stopwords | gensim_stopwords
...
def create_pattern(nlp, doc, debug=False):
    ...
    if len(doc) >= 5 or doc[0].text in pronoun_list or doc[-1].text in pronoun_list or \
            all([(token.text in merged_stopwords or token.lemma_ in merged_stopwords or token.lemma_ in blacklist) for token in doc]):
        if debug:
    ...
    for item in tqdm(data):
        ...
        for c in qc:
            ...
            if c[-1:] == "e" and c[:-1] in qc:
                continue
            have_stop = all(t in merged_stopwords or t in blacklist for t in c.split("_"))
        ...
        for c in ac:
            ...
            if c[-1:] == "e" and c[:-1] in ac:
                continue
            all_stop = all(t in merged_stopwords for t in c.split("_"))
...
def ground(statement_path, cpnet_vocab_path, pattern_path, output_path, num_processes=1, debug=False):
    ...
    if res is None:
        print("❌ match_mentioned_concepts() returned None!")
